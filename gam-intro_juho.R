...
gtemp <- read_delim(URL, delim = ' ', col_types = 'nnnnnnnnnnnn', col_names = FALSE) %>%
    select(num_range('X', c(1,4))) %>% setNames(nm = c('Year', 'Temperature')) ### which columns?

read_delim(URL, delim = ' ', col_types = 'nnnnnnnnnnnn', col_names = FALSE)
View(gtemp)

gbtemp <- read_csv("/home/pietila/FMI/intro-gam-webinar-2020-master/data/gbtemp.csv")
View(gbtemp)
